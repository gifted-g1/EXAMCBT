...
type FaceVerification struct {
	...
	StudentID   uuid.UUID  `gorm:"type:char(36);index;not null" json:"student_id"`
	ExamID      uuid.UUID  `gorm:"type:char(36);index" json:"exam_id,omitempty"`
	AttemptID   *uuid.UUID `gorm:"type:char(36);index" json:"attempt_id,omitempty"`
...
type MonitoringSession struct {
	ID                uuid.UUID  `gorm:"type:char(36);primaryKey" json:"id"`
	AttemptID         uuid.UUID  `gorm:"type:char(36);index;not null" json:"attempt_id"`
	StudentID         uuid.UUID  `gorm:"type:char(36);index;not null" json:"student_id"`
	ExamID            uuid.UUID  `gorm:"type:char(36);index;not null" json:"exam_id"`
	CameraActive      bool       `gorm:"default:false" json:"camera_active"`
	ConnectionQuality string     `gorm:"type:varchar(20);default:'UNKNOWN'" json:"connection_quality"` // GOOD/POOR/DISCONNECTED
...
type AuditLog struct {
	ID         uuid.UUID `gorm:"type:char(36);primaryKey" json:"id"`
	ActorID    uuid.UUID `gorm:"type:char(36);index" json:"actor_id"`
	ActorRole  Role      `gorm:"type:varchar(30);index" json:"actor_role"`
	...
	IPAddress  string    `gorm:"type:varchar(45)" json:"ip_address,omitempty"`
	Metadata   string    `gorm:"type:text" json:"metadata,omitempty"` // JSON-encoded free-form context
	Timestamp  time.Time `gorm:"index" json:"timestamp"`
}
